    ...
    return (
      <div style={{display:"flex",gap:8,alignItems:"center"}}>
        {LANGS.map((l:any)=>{
          ...
          return (
            <button key={l.code} onClick={()=>setLang(l.code)} style={{padding:"6px 10px",borderRadius:12,border: selected? "1px solid #c9a84c" : (dark? "1px solid rgba(255,255,255,0.08)" : "1px solid rgba(0,0,0,0.08)"),background:selected?"#c9a84c":"transparent",color:selected?"#1a1a1a":(dark?"rgba(255,255,255,0.85)":"#1a1a1a"),fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>{l.name}</button>
          );
        })}
      </div>
    );
  ...
  return (
    <div style={{display:"flex",gap:10,alignItems:"center"}}>
      {LANGS.map((l:any)=>{
        const selected = lang===l.code;
        return (
          <button key={l.code} onClick={()=>setLang(l.code)} style={{padding:"8px 12px",borderRadius:12,border: selected? "1px solid #c9a84c" : (dark? "1px solid rgba(255,255,255,0.08)" : "1px solid rgba(0,0,0,0.08)"),background:selected?"#c9a84c":"transparent",color:selected?"#1a1a1a":(dark?"rgba(255,255,255,0.85)":"#1a1a1a"),fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>{l.name}</button>
        );
      })}
    </div>
  );
...
function AuthScreen({mode,onLogin,onBack,lang,setLang,users,setUsers}:any){
  ...
  const doLogin=()=>{
    ...
    setTimeout(()=>{
      setLoading(false);
      const u=users.find((x:any)=>x.email.trim().toLowerCase()===trimEmail&&x.password===trimPwd);
  ...
  const doRegister=()=>{
    ...
    if(users.find((u:any)=>u.email===rEmail)){setErr("Email already registered.");return;}
    setLoading(true);
    setTimeout(()=>{
      setLoading(false);
      setUsers((u:any)=>[...u,{id:`u${Date.now()}`,name:rName,email:rEmail,password:rPwd,role:"user",status:"pending",regiment:rRegiment}]);
              ...
              <>
                ...
                {[{label:t.email,val:email,set:setEmail,type:"email",ph:"Enter email"},{label:t.password,val:pwd,set:setPwd,type:"password",ph:"••••••••"}].map(f=>(
                  <div key={f.label} style={{marginBottom:16}}>
                    ...
                    <input type={f.type} value={f.val} onChange={(e:any)=>f.set(e.target.value)} placeholder={f.ph} onKeyDown={(e:any)=>e.key==="Enter"&&doLogin()} style={{width:"100%",padding:"13px 15px",border:"2px solid #e5e7eb",borderRadius:12,fontSize:15,outline:"none",fontFamily:"'DM Sans',sans-serif",color:"#1a1a1a",boxSizing:"border-box"}} onFocus={(e:any)=>e.target.style.borderColor="#1F3D2B"} onBlur={(e:any)=>e.target.style.borderColor="#e5e7eb"}/>
                  </div>
                ))}
              ...
              <>
                ...
                {[{label:t.name,val:rName,set:setRName,type:"text",ph:"Full Name"},{label:t.email,val:rEmail,set:setREmail,type:"email",ph:"Email"},{label:t.choosePwd,val:rPwd,set:setRPwd,type:"password",ph:"Password"}].map(f=>(
                  <div key={f.label} style={{marginBottom:14}}>
                    <label style={{display:"block",fontSize:12,fontWeight:600,color:"#555",marginBottom:6}}>{f.label}</label>
                    <input type={f.type} value={f.val} onChange={(e:any)=>f.set(e.target.value)} placeholder={f.ph} style={{width:"100%",padding:"13px 15px",border:"2px solid #e5e7eb",borderRadius:12,fontSize:15,outline:"none",fontFamily:"'DM Sans',sans-serif",color:"#1a1a1a",boxSizing:"border-box"}}/>
                  </div>
                ))}
...
function UserDashboard({onNav,lang}:any){
  const{currentUser,bookings,slots}=useApp(), t=(T as any)[lang];
  const my=bookings.filter((b:any)=>b.userId===currentUser.id), up=my.filter((b:any)=>b.status==="active")[0];
  const today=new Date().toISOString().split("T")[0], avail=slots.filter((s:any)=>s.date===today&&!s.disabled&&s.booked<s.capacity).length;
  return(
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:13,marginBottom:24}}>
        {[{l:"Total Bookings",v:my.length,i:"🎫",c:"#1F3D2B"},{l:"Active Tokens",v:my.filter((b:any)=>b.status==="active").length,i:"✅",c:"#065f46"},{l:"Available Today",v:avail,i:"🕐",c:"#92400e"}].map((s:any)=>(
...
function BookSlot({lang}:any){
  const{currentUser,slots,addBooking,bookings,showToast}=useApp(), t=(T as any)[lang];
  const[step,setStep]=useState(1), [cat,setCat]=useState(""), [selDate,setSelDate]=useState(""), [selSlot,setSelSlot]=useState<any>(null);
  const today=new Date();
  const dates=Array.from({length:14},(_,i)=>{const d=new Date(today);d.setDate(today.getDate()+i);return d.toISOString().split("T")[0];});
  const dateSlots=selDate?slots.filter((s:any)=>s.date===selDate):[];
  const alreadyBooked=selDate&&bookings.some((b:any)=>b.userId===currentUser.id&&b.date===selDate&&b.status==="active");
    ...
    <div>
      ...
      {step===1&&(
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(170px,1fr))",gap:13}}>
          {[{id:"Grocery",icon:"🛒",desc:"Open to all"},{id:"Liquor Only",icon:"🥃",desc:"Registered members"},{id:"Grocery + Liquor",icon:"🛒🥃",desc:"Combined"}].map((c:any)=>(
      ...
      {step===2&&(
        <div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(96px,1fr))",gap:9}}>
            {dates.map((d:any)=>{
              const ds=slots.filter((s:any)=>s.date===d), isHol=ds.every((s:any)=>s.disabled), isFull=!isHol&&ds.every((s:any)=>s.booked>=s.capacity);
              const av=ds.filter((s:any)=>!s.disabled&&s.booked<s.capacity).length, date=new Date(d), isSel=selDate===d, dis=isHol||isFull;
              return(
...
function MyBookings({lang}:any){
  const{currentUser,bookings,cancelBooking,showToast}=useApp(), t=(T as any)[lang];
...
function AdminDashboard(){
  ...
  const tc=slots.filter((s:any)=>s.date===today).reduce((a:any,s:any)=>a+s.capacity,0);
  const tb2=slots.filter((s:any)=>s.date===today).reduce((a:any,s:any)=>a+s.booked,0);
  ...
  return(
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:13,marginBottom:24}}>
        {[{l:"Today's Bookings",v:tb.length,i:"📅",c:"#1F3D2B"},{l:"Total Users",v:users.length,i:"👥",c:"#1e40af"},{l:"Pending Approval",v:users.filter((u:any)=>u.status==="pending").length,i:"⏳",c:"#92400e"},{l:"Active Tokens",v:bookings.filter((b:any)=>b.status==="active").length,i:"✅",c:"#065f46"}].map((s:any)=>(
      ...
      <div style={{background:"var(--card)",borderRadius:14,padding:20,boxShadow:"var(--shadow)"}}>
        ...
        {["Grocery","Liquor Only","Grocery + Liquor"].map((c:any,i:any)=>{
          const cnt=bookings.filter((b:any)=>b.category===c).length, pct=bookings.length?Math.round((cnt/bookings.length)*100):0;
          return(
...
function AdminSlots(){
  ...
  const[editSlot,setEditSlot]=useState<any>(null), [newCap,setNewCap]=useState("");
  const uDates=[...new Set(slots.map((s:any)=>s.date))].slice(0,14);
  const dSlots=slots.filter((s:any)=>s.date===selDate);
  return(
    <div>
      ...
      {editSlot&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.4)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000,padding:20}} onClick={()=>setEditSlot(null)}>
          <div onClick={(e:any)=>e.stopPropagation()} style={{background:"var(--card)",borderRadius:18,padding:26,maxWidth:320,width:"100%",boxShadow:"0 24px 64px rgba(0,0,0,0.25)"}}>
            ...
            <input type="number" value={newCap} onChange={(e:any)=>setNewCap(e.target.value)} style={{width:"100%",padding:"11px 13px",border:"2px solid var(--border)",borderRadius:10,fontSize:15,outline:"none",color:"var(--text)",background:"var(--bg)",boxSizing:"border-box",fontFamily:"'DM Sans',sans-serif",marginBottom:16}}/>
...
function AdminBookings(){
  ...
  let fil=sf==="all"?bookings:bookings.filter((b:any)=>b.status===sf);
  if(cf!=="all") fil=fil.filter((b:any)=>b.category===cf);
  const sc:any={active:{bg:"#dcfce7",tx:"#166534"},completed:{bg:"#dbeafe",tx:"#1e40af"},cancelled:{bg:"#fee2e2",tx:"#991b1b"}};
  return(
    <div>
      ...
      <div style={{display:"flex",gap:7,flexWrap:"wrap",marginBottom:16,alignItems:"center"}}>
        ...
        <select value={cf} onChange={(e:any)=>setCf(e.target.value)} style={{marginLeft:"auto",padding:"6px 13px",borderRadius:18,border:"2px solid var(--border)",background:"var(--card)",color:"var(--text)",fontSize:11,outline:"none",fontFamily:"'DM Sans',sans-serif"}}>
        ...
        <table style={{width:"100%",borderCollapse:"collapse",minWidth:560}}>
          ...
          <tbody>
            {users.map((u:any,i:any)=>{
              const sc:any={active:{bg:"#dcfce7",tx:"#166534"},pending:{bg:"#fef3c7",tx:"#92400e"},disabled:{bg:"#fee2e2",tx:"#991b1b"}}[u.status]||{};
              return(
...
export default function App(){
  ...
  const showToast=useCallback((msg:any,type="success")=>{const id=Date.now();setToasts((t:any)=>[...t,{id,message:msg,type}]);setTimeout(()=>setToasts((t:any)=>t.filter((x:any)=>x.id!==id)),3500);},[]);
  const removeToast=useCallback((id:any)=>setToasts((t:any)=>t.filter((x:any)=>x.id!==id)),[]);
  const addBooking=useCallback((b:any,sid:any)=>{setBookings((x:any)=>[...x,b]);setSlots((s:any)=>s.map((sl:any)=>sl.id===sid?{...sl,booked:sl.booked+1}:sl));},[]);
  const cancelBooking=useCallback((id:any,sid:any)=>{setBookings((b:any)=>b.map((x:any)=>x.id===id?{...x,status:"cancelled"}:x));setSlots((s:any)=>s.map((sl:any)=>sl.id===sid?{...sl,booked:Math.max(0,sl.booked-1)}:sl));},[]);
  const updateSlot=useCallback((id:any,u:any)=>setSlots((s:any)=>s.map((sl:any)=>sl.id===id?{...sl,...u}:sl)),[]);
  const updateUser=useCallback((id:any,u:any)=>setUsers((x:any)=>x.map((usr:any)=>usr.id===id?{...usr,...u}:usr)),[]);
  ...
  if(screen==="auth-user"||screen==="auth-admin"){
    return <AuthScreen mode={screen==="auth-admin"?"admin":"user"} onLogin={(u:any)=>{setCurrentUser(u);setPage(u.role==="admin"?"admin-dashboard":"dashboard");setScreen("app");}} onBack={()=>setScreen("home")} lang={lang} setLang={setLang} users={users} setUsers={setUsers}/>;
  }

  const uPages:any={dashboard:<UserDashboard onNav={setPage} lang={lang}/>,book:<BookSlot lang={lang}/>,bookings:<MyBookings lang={lang}/>,profile:<UserProfile/>};
  const aPages:any={"admin-dashboard":<AdminDashboard/>,"admin-slots":<AdminSlots/>,"admin-bookings":<AdminBookings/>,"admin-users":<AdminUsers/>};
  ...
  return(
    <Ctx.Provider value={{currentUser,slots,bookings,users,addBooking,cancelBooking,updateSlot,updateUser,showToast}}>
      ...
      <div style={{display:"flex",minHeight:"100vh",background:`${vars.bg}`,"--bg":vars.bg,"--card":vars.card,"--text":vars.text,"--muted":vars.muted,"--border":vars.border,"--shadow":vars.shadow} as any}>
        {isAdmin
        ...
        <main style={{marginLeft:230,flex:1,padding:"32px 32px 60px",maxWidth:"calc(100vw - 230px)",overflowX:"hidden","--bg":vars.bg,"--card":vars.card,"--text":vars.text,"--muted":vars.muted,"--border":vars.border,"--shadow":vars.shadow} as any}>
          <div style={{display:"flex",justifyContent:"flex-end",marginBottom:20,gap:8,alignItems:"center"}}>
            <button onClick={()=>setDark((d:any)=>!d)} style={{background:vars.card,border:`2px solid ${vars.border}`,borderRadius:17,padding:"7px 14px",cursor:"pointer",fontSize:12,color:vars.text,fontFamily:"'DM Sans',sans-serif",boxShadow:vars.shadow}}>{dark?"☀️ Light":"🌙 Dark"}</button>
          </div>
