...
// ── Separate dashboard files (lazy-loaded for faster initial paint) ──────────
const UserDashboard = lazy(() => import("./UserDashboard"));
const AdminDashboard = lazy(() => import("./AdminDashboard").then(m => ({ default: m.default })));
const AdminSlots = lazy(() => import("./AdminDashboard").then(m => ({ default: m.AdminSlots })));
const AdminTokens = lazy(() => import("./AdminDashboard").then(m => ({ default: m.AdminTokens })));
const AdminUsers = lazy(() => import("./AdminDashboard").then(m => ({ default: m.AdminUsers })));
// ─────────────────────────────────────────────────────────────────────────────
...
export default function App() {
  ...
  const uPages: any = {
    dashboard: <UserDashboard onNav={handleNav} lang={lang} t={(T as any)[lang]} />,
  ...
  const aPages: any = {
    "admin-dashboard": <AdminDashboard lang={lang} onNav={handleNav} />,
  ...
  return (
    <Ctx.Provider value={{ currentUser, slots, bookings, users, notices, showToast, updateSlots: setSlots, updateUsers: setUsers, updateBookings: setBookings, updateNotices: setNotices }}>
      ...
      >
        {adminFlag
          ? <AdminSidebar active={page} onNav={handleNav} user={currentUser} onLogout={() => { setCurrentUser(null); setScreen("home"); }} dark={dark} mobile={isMobile} open={!isMobile || sidebarOpen} onClose={() => setSidebarOpen(false)} />
          : <UserSidebar active={page} onNav={handleNav} user={currentUser} onLogout={() => { setCurrentUser(null); setScreen("home"); }} dark={dark} lang={lang} setLang={setLang} mobile={isMobile} open={!isMobile || sidebarOpen} onClose={() => setSidebarOpen(false)} />
        }
        ...
        >
          ...
          <Suspense fallback={
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", minHeight: "50vh", color: "var(--muted)", fontSize: 14 }}>
              <div style={{ textAlign: "center" }}>
                <div style={{ width: 40, height: 40, border: "3px solid var(--border)", borderTop: "3px solid #1F3D2B", borderRadius: "50%", animation: "spin 0.8s linear infinite", margin: "0 auto 16px" }} />
            ...
            {adminFlag
              ? (aPages[page] || <AdminDashboard lang={lang} onNav={handleNav} />)
              : (uPages[page] || <UserDashboard onNav={handleNav} lang={lang} t={(T as any)[lang]} />)
            }
          </Suspense>
        </main>
      </div>
